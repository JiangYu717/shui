          ...
          <div class="upload-area">
            ...
            <div class="import-info">
              <el-alert
                ...
                show-icon
              >
                <p>• 批量导入会自动跳过重复的监测点名称</p>
                <p>• 只导入不重复的数据，提高导入成功率</p>
                <p>• 导入完成后会显示跳过的重复监测点名称</p>
              </el-alert>
            </div>
          </div>
        </el-tab-pane>
...
import { importSectionMonitor, batchImportSectionMonitor, downloadTemplate as downloadTemplateApi } from '@/api/sectionMonitor'
...
const handleSuccess = (response, file, importType) => {
  ...
  if (response?.data) {
    ...
    if (result.skippedReservoirs && result.skippedReservoirs.length > 0) {
      status = '部分成功'
      message += `，跳过 ${result.skippedReservoirs.length} 个重复监测点：${result.skippedReservoirs.join(', ')}`
    }
  } else {
...
const customUpload = async ({ file }) => {
  try {
    const response = await importSectionMonitor(file)
...
const batchUpload = async ({ file }) => {
  try {
    const response = await batchImportSectionMonitor(file)
